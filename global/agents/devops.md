...
tools: Read, Write, Edit, Bash, Grep, mcp__supabase, mcp__firebase, mcp__github
memory: user
model: sonnet
---
...
- **Web**: `~/.claude/scripts/preview-deploy.sh <project> <branch-or-pr-number> <port>`
...
- **Teardown**: when the orchestrator tells you the PR merged or closed, run
  `~/.claude/scripts/preview-teardown.sh <project> <branch-or-pr-number>`
