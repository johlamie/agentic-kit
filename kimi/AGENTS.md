...
Kimi Code has no custom subagents: each role below is a **skill** in
`~/.kimi-code/skills/<role>/SKILL.md`. To run a role, dispatch a built-in
`coder` subagent whose prompt instructs it to invoke the role's skill (see
"Delegation rules").
...
- Roles run as built-in `coder` subagents. In the dispatch prompt, order the
  subagent to invoke the role's skill and take the role, e.g.: "invoke skill
  `builder` — you are the builder for slice X". `coder` subagents can invoke
  skills and can run in parallel.
...
- Skills do not auto-load agent memory. Point the subagent at its memory file
  in the dispatch prompt and remind it to update that file: user-scope roles
  (product-manager, researcher, designer, devops) use
  `~/.kimi-code/agent-memory/<role>/MEMORY.md` (index plus topic files in the
  same directory, layout imported from Claude Code); project-scope roles
  (architect, builder, reviewer, qa) use `.kimi-code/agent-memory/<role>.md`.
...
## MCP toolbox (verify availability with /mcp-config before relying on it)
...
retrospective. Propose AGENTS.md/skill amendments as diffs — never self-edit rules.
